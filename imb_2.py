...
scaled_df = pd.DataFrame(rob_scaler.fit_transform(df[num_cols]), columns=num_cols)
cat_df = df[cat_cols].astype("category")
cat_df.reset_index(drop=True, inplace=True)
target = pd.DataFrame(df["target"].reset_index(drop=True))
new_df = pd.concat([scaled_df, cat_df, target], axis=1)
...
df_1 = df.copy()
# new_df = pd.concat([new_df, df["target"].reset_index(drop=True)])
new_df = new_df.sample(frac=1)
df = new_df.copy()

# amount of fraud classes 492 rows.
fraud_df = df.loc[df['target'] == 1]
non_fraud_df = df.loc[df['target'] == 0][:4494]
...
sns.boxplot(x="target", y="num12", data=new_df, palette=colors, ax=axes[0])
axes[0].set_title('num12 vs Class Negative Correlation')

sns.boxplot(x="target", y="num13", data=new_df, palette=colors, ax=axes[1])
axes[1].set_title('num13 vs Class Negative Correlation')


sns.boxplot(x="target", y="num14", data=new_df, palette=colors, ax=axes[2])
axes[2].set_title('num14 vs Class Negative Correlation')


sns.boxplot(x="target", y="num15", data=new_df, palette=colors, ax=axes[3])
axes[3].set_title('num15 vs Class Negative Correlation')
...
v14_fraud_dist = new_df['num14'].loc[new_df['target'] == 1].values
sns.distplot(v14_fraud_dist,ax=ax1, fit=norm, color='#FB8861')
ax1.set_title('Num14 Distribution \n (Fraud Transactions)', fontsize=14)

v12_fraud_dist = new_df['num12'].loc[new_df['target'] == 1].values
sns.distplot(v12_fraud_dist,ax=ax2, fit=norm, color='#56F9BB')
ax2.set_title('num12 Distribution \n (Fraud Transactions)', fontsize=14)


v13_fraud_dist = new_df['num13'].loc[new_df['target'] == 1].values
sns.distplot(v13_fraud_dist,ax=ax3, fit=norm, color='#C5B3F9')
ax3.set_title('num13 Distribution \n (Fraud Transactions)', fontsize=14)

v15_fraud_dist = new_df['num15'].loc[new_df['target'] == 1].values
sns.distplot(v15_fraud_dist,ax=ax4, fit=norm, color='#C5B3F9')
ax4.set_title('num15 Distribution \n (Fraud Transactions)', fontsize=14)
...
undersample_X = df.drop('target', axis=1)

undersample_y = df['target']
